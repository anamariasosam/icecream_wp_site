...
class SweetToothElatedClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_sweettooth_elated_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_sweettooth_elated_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'sweettooth' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'sweettooth' );
		}
...
if ( ! function_exists( 'sweettooth_elated_create_like' ) ) {
	function sweettooth_elated_create_like() {
		SweetToothElatedClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'sweettooth_elated_create_like' );
}