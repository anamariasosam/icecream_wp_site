 ...
 * FUNCTIONS LIST
 * 1)  @see sweettooth_elated_get_blog_module_name
 * 2)  @see sweettooth_elated_include_blog_helper_functions
 * 3)  @see sweettooth_elated_get_blog_single_layout
 * 4)  @see sweettooth_elated_get_archive_blog_list_layout
 * 5)  @see sweettooth_elated_get_holder_params_blog
 * 6)  @see sweettooth_elated_get_blog
 * 7)  @see sweettooth_elated_get_blog_type
 * 8)  @see sweettooth_elated_get_blog_query
 * 9)  @see sweettooth_elated_get_blog_list_holder_classes
 * 10) @see sweettooth_elated_get_blog_holder_data_params
 * 11) @see sweettooth_elated_set_ajax_url
 * 12) @see sweettooth_elated_blog_load_more
 * 13) @see sweettooth_elated_get_post_format_html
 * 14) @see sweettooth_elated_single_link_pages
 * 15) @see sweettooth_elated_get_blog_single
 * 16) @see sweettooth_elated_get_blog_single_type
 * 17) @see sweettooth_elated_get_single_post_format_html
 * 18) @see sweettooth_elated_text_crop
 * 19) @see sweettooth_elated_excerpt
 * 20) @see sweettooth_elated_excerpt_length
 * 21) @see sweettooth_elated_post_has_read_more
 * 22) @see sweettooth_elated_modify_read_more_link
 * 23) @see sweettooth_elated_get_blog_related_post_type
 * 24) @see sweettooth_elated_get_blog_related_posts
 * 25) @see sweettooth_elated_blog_shortcode_load_more
 * 26) @see sweettooth_elated_get_user_custom_fields
 * 27) @see sweettooth_elated_blog_item_has_link
 * 28) @see sweettooth_elated_get_blog_module
 * 29) @see sweettooth_elated_return_post_format
 * 30) @see sweettooth_elated_return_has_media
 * 31) @see sweettooth_elated_blog_single_title
 * 32) @see sweettooth_elated_blog_single_title_params
 * 34) @see sweettooth_elated_post_has_thumbnail

**/

if(!function_exists('sweettooth_elated_get_blog_module_name')) {
    /**
     ...
     *
     */
    function sweettooth_elated_get_blog_module_name() {
...
if( !function_exists('sweettooth_elated_include_blog_helper_functions') ) {
	/**
     ...
     *
	 */
	function sweettooth_elated_include_blog_helper_functions($module, $type) {
		include_once ELATED_FRAMEWORK_MODULES_ROOT_DIR.'/blog/templates/'. $module .'/' . $type . '/helper.php';
	}
}

if( !function_exists('sweettooth_elated_get_blog_single_layout') ) {
    /**
     * Function which return archive blog list layout
     */
    function sweettooth_elated_get_blog_single_layout() {
        $blog_single_layout = sweettooth_elated_get_meta_field_intersect('blog_single_type');

        return $blog_single_layout;
    }
}

if( !function_exists('sweettooth_elated_get_archive_blog_list_layout') ) {
	/**
	 * Function which return archive blog list layout
	 */
	function sweettooth_elated_get_archive_blog_list_layout() {
		$blog_layout = sweettooth_elated_options()->getOptionValue('blog_list_type');
		
		return $blog_layout;
	}
}

if( !function_exists('sweettooth_elated_get_holder_params_blog') ) {
	/**
	 * Function which return holder class and holder inner class for blog pages
	 *
	 * @return array
	 */
	function sweettooth_elated_get_holder_params_blog() {
		$params = array();
         ...
         * -inner
         */
        $params = apply_filters('sweettooth_elated_filter_blog_holder_params', $params);

		return $params;
	}
}

if( !function_exists('sweettooth_elated_get_blog') ) {
	/**
	 * Function which return holder for all blog lists
	 *
	 *
	 */
	function sweettooth_elated_get_blog($type) {
		$holder_classes = '';
		$holder_classes = apply_filters('sweettooth_elated_filter_blog_holder_classes', $holder_classes);
		...
		sweettooth_elated_get_module_template_part('templates/lists/holder', 'blog', '', $params);
	}
}

if( !function_exists('sweettooth_elated_get_blog_type') ) {
	/**
	 * Function which create query for blog lists
	 * @param $type string with name of list that is loaded
	 *
	 */
	function sweettooth_elated_get_blog_type($type) {
        $blog_query = sweettooth_elated_get_blog_query();
		$paged = isset($blog_query->query['paged']) ? $blog_query->query['paged'] : 1;
		$max_num_pages = $blog_query->max_num_pages;
		
        $blog_classes = sweettooth_elated_get_blog_list_holder_classes($type);
        $blog_data_params = sweettooth_elated_get_blog_holder_data_params($type);
		...
		sweettooth_elated_get_module_template_part('templates/lists/' .  $type . '/list', 'blog', '', $params);
	}
}

if(!function_exists('sweettooth_elated_get_blog_query')){
	/**
	* Function which create query for blog lists
	*
	* @return wp query object
	*/
	function sweettooth_elated_get_blog_query(){
...
if(!function_exists('sweettooth_elated_get_max_number_of_pages')) {
    /**
     ...
     * @version 0.1
     */
    function sweettooth_elated_get_max_number_of_pages() {
        global $wp_query;
...
if(!function_exists('sweettooth_elated_get_blog_list_holder_classes')){
	/**
	 * Function set blog list classes
	 * @param $type - type of blog list that is passing
	 * @return string - string with formatted classes
	 */
	function sweettooth_elated_get_blog_list_holder_classes($type){
		$blog_classes   = array();
		$blog_classes[] = 'eltdf-blog-holder';
		$blog_classes[] = 'eltdf-blog-' . $type;
		
		$pagination_type = sweettooth_elated_get_meta_field_intersect('blog_pagination_type');
		if(!empty($pagination_type)) {
			$blog_classes[] = 'eltdf-blog-pagination-' . $pagination_type;
		}
        $masonry_type = sweettooth_elated_get_meta_field_intersect('blog_list_featured_image_proportion');
        if(!empty($masonry_type)) {
            $blog_classes[] = 'eltdf-masonry-images-' . $masonry_type;
        }
		
		$blog_classes = apply_filters('sweettooth_elated_filter_blog_list_classes', $blog_classes);
		
		return implode(' ', $blog_classes);
	}
}

if(!function_exists('sweettooth_elated_get_blog_holder_data_params')){
	/**
	 ...
	 */
	function sweettooth_elated_get_blog_holder_data_params($type){
		$current_query = sweettooth_elated_get_blog_query();
		...
		$category = get_post_meta(sweettooth_elated_get_page_id(), 'eltdf_blog_category_meta', true);
        $excerpt_length = sweettooth_elated_get_meta_field_intersect('number_of_chars', sweettooth_elated_get_page_id());
...
if(!function_exists('sweettooth_elated_blog_load_more')){
	
	function sweettooth_elated_blog_load_more(){
		$return_obj = array();
        $params = array();
		...
		if($query_results->have_posts()):
			while ( $query_results->have_posts() ) : $query_results->the_post();
				$html .= sweettooth_elated_get_post_format_html($blog_type, 'yes', $params);
			endwhile;
		else:
			$html .= sweettooth_elated_get_module_template_part('templates/parts/no-posts', 'blog');
		endif;
    ...
    add_action('wp_ajax_nopriv_sweettooth_elated_blog_load_more', 'sweettooth_elated_blog_load_more');
    add_action( 'wp_ajax_sweettooth_elated_blog_load_more', 'sweettooth_elated_blog_load_more' );
}

if( !function_exists('sweettooth_elated_get_post_format_html') ) {
	/**
	 ...
	 */
	function sweettooth_elated_get_post_format_html($type = "", $ajax = '', $ajax_params = array()) {

        $post_format = sweettooth_elated_return_post_format();
        ...
        $post_classes[] = sweettooth_elated_return_has_media() ? 'eltdf-post-has-media' : 'eltdf-post-no-media';
        ...
        $params['part_params']  = apply_filters('sweettooth_elated_filter_blog_part_params', $part_params_temp);

		if($ajax == ''){
			sweettooth_elated_get_module_template_part('templates/lists/' . $type . '/post', 'blog', $post_format, $params);
		}
		if($ajax == 'yes'){
			return sweettooth_elated_get_blog_module_template_part('templates/lists/' . $type . '/post', $post_format, $params);
		}
	}
}

if( !function_exists('sweettooth_elated_single_link_pages') ) {
    /**
     * Function which return parts on single.php which are just below content
     *
     *
     */
    function sweettooth_elated_single_link_pages() {
    ...
    add_action('sweettooth_elated_action_single_link_pages', 'sweettooth_elated_single_link_pages');
}

if(!function_exists('sweettooth_elated_get_blog_single')) {
	/**
	 ...
	 *
	 */
	function sweettooth_elated_get_blog_single($type) {
		$sidebar_layout = sweettooth_elated_sidebar_layout();
		
		$holder_classes = $sidebar_layout !== 'no-sidebar' ? 'eltdf-content-has-sidebar' : '';
		$holder_classes = apply_filters('sweettooth_elated_filter_blog_single_holder_classes', $holder_classes);
		...
		sweettooth_elated_get_module_template_part('templates/singles/holder', 'blog', '', $params);
	}
}

if( !function_exists('sweettooth_elated_get_blog_single_type') ) {
    /**
     * Function which returns proper single post template
     * @param $type string with name of list that is loaded
     *
     */
    function sweettooth_elated_get_blog_single_type($type) {
         ...
         *
         */
        $params['single_info_params'] = apply_filters('sweettooth_elated_filter_blog_single_info_params', array());

        sweettooth_elated_get_module_template_part('templates/singles/'.$type.'/single', 'blog', '', $params);
    }
}

if( !function_exists('sweettooth_elated_get_single_post_format_html') ) {
	/**
	 ...
	 *
	 */
	function sweettooth_elated_get_single_post_format_html($type) {
        $post_format = sweettooth_elated_return_post_format();
		$params = array();
        /*
         * Available parameters for template parts
         ...
         *
         */
        $params['part_params']  = apply_filters('sweettooth_elated_filter_blog_part_params', array());

        sweettooth_elated_get_module_template_part('templates/singles/'.$type.'/'.$post_format, 'blog', '', $params);
	}
}

if(!function_exists('sweettooth_elated_text_crop')) {
    /**
     * Function that cuts plain text to the number of words supplied
     *
     */
    function sweettooth_elated_text_crop($text, $word_count = 45, $suffix = true) {
...
if(!function_exists('sweettooth_elated_excerpt')) {
    /**
     ...
     *
     */
	function sweettooth_elated_excerpt( $length ) {
		global $post;
		
		if ( sweettooth_elated_post_has_read_more() ) {
			global $more;
		...
		$number_of_chars = sweettooth_elated_get_meta_field_intersect( 'number_of_chars', sweettooth_elated_get_page_id() );
...
if (!function_exists('sweettooth_elated_excerpt_length')) {
	/**
	 * Function that changes excerpt length based on theme options
	 * @return int changed value
	 */
	function sweettooth_elated_excerpt_length() {
		$numb_of_chars = sweettooth_elated_options()->getOptionValue('number_of_chars');
		
		return $numb_of_chars !== '' ? $numb_of_chars : 45;
	}

	add_filter( 'excerpt_length', 'sweettooth_elated_excerpt_length', 999 );
}

if(!function_exists('sweettooth_elated_post_has_read_more')) {
	/**
	 * Function that checks if current post has read more tag set
	 * @return int position of read more tag text. It will return false if read more tag isn't set
	 */
	function sweettooth_elated_post_has_read_more() {
		global $post;
...
if (!function_exists('sweettooth_elated_modify_read_more_link')) {
	/**
	 * Function that modifies read more link output.
	 * Hooks to the_content_more_link
	 * @return string modified output
	 */
	function sweettooth_elated_modify_read_more_link() {
		$link = '<div class="eltdf-more-link-container">';
        if(sweettooth_elated_core_plugin_installed()) {
            $link .= sweettooth_elated_get_button_html(array(
                'link' => get_permalink() . '#more-' . get_the_ID(),
                'text' => esc_html__('Continue reading', 'sweettooth')
            ));
        } else {
            $link .='<a itemprop="url" href="' . get_permalink() . '#more-' . get_the_ID() . '" target="_self" class="eltdf-btn eltdf-btn-small eltdf-btn-solid">';
            $link .= '<span class="eltdf-btn-text">';
            $link .= esc_html__('Continue reading', 'sweettooth');
            $link .= '</span></a>';
        }
	...
	add_filter( 'the_content_more_link', 'sweettooth_elated_modify_read_more_link');
}

if ( ! function_exists('sweettooth_elated_get_blog_related_post_type')) {
    /**
     ...
     * @return WP_Query
     */
    function sweettooth_elated_get_blog_related_post_type($post_id, $options = array()) {
        ...
        if ($tag_ids) {
            $related_by_tag = sweettooth_elated_get_blog_related_posts($post_id, $tag_ids, 'tag', $options);
        ...
        if ($categories && !$hasRelatedByTag) {
            $related_by_category = sweettooth_elated_get_blog_related_posts($post_id, $category_ids, 'category', $options);
...
if ( ! function_exists('sweettooth_elated_get_blog_related_posts') ) {
    /**
     ...
     * @return WP_Query
     */
    function sweettooth_elated_get_blog_related_posts($post_id, $term_ids, $slug, $options = array()) {
        //Query options
        $posts_per_page = -1;
...
if(!function_exists('sweettooth_elated_blog_shortcode_load_more')){

    function sweettooth_elated_blog_shortcode_load_more(){
        $params = array();
        ...
        if($query_results->have_posts()):
            while ( $query_results->have_posts() ) : $query_results->the_post();
                sweettooth_elated_get_module_template_part('shortcodes/blog-list/layout-collections/'.$params['type'], 'blog', '', $params);
            endwhile;
        else:
    ...
    add_action('wp_ajax_nopriv_sweettooth_elated_blog_shortcode_load_more', 'sweettooth_elated_blog_shortcode_load_more');
    add_action( 'wp_ajax_sweettooth_elated_blog_shortcode_load_more', 'sweettooth_elated_blog_shortcode_load_more' );
}

if(!function_exists('sweettooth_elated_get_user_custom_fields')) {
	/**
	 * Function returns links and icons for author social networks
	 *
	 * return array
	 */
	function sweettooth_elated_get_user_custom_fields() {
...
if(!function_exists('sweettooth_elated_blog_item_has_link')) {
	/**
	 * Function returns true/false depends is single post or in loop
	 *
	 * return boolean
	 */
	function sweettooth_elated_blog_item_has_link() {
		$is_link = (is_single() && (get_the_ID() === sweettooth_elated_get_page_id())) ? false : true;

		return $is_link;
	}
}

if(!function_exists('sweettooth_elated_get_blog_module')) {
    /**
     * Function returns single/list depending is single post or in loop
     *
     * return string
     */
    function sweettooth_elated_get_blog_module() {
        $module = (is_single() && (get_the_ID() === sweettooth_elated_get_page_id())) ? 'single' : 'list';

        return $module;
    }
}

if( !function_exists('sweettooth_elated_return_post_format') ) {
    /**
     * Function return all parts on single.php page
     *
     * @return string with post format
     */
    function sweettooth_elated_return_post_format() {
...
if( !function_exists('sweettooth_elated_return_has_media') ) {
    /**
     * Function return all parts on single.php page
     *
     * @return string with post format
     */
    function sweettooth_elated_return_has_media() {
        $post_format = get_post_format();
...
if(!function_exists('sweettooth_elated_blog_single_title')) {
    /**
     * Function that checks option for single product title and overrides it with filter
     * @param $show_title_area - boolean, default value from title function @see sweettooth_elated_get_title()
     * @return boolean
     */
    function sweettooth_elated_blog_single_title($show_title_area) {
        if(is_singular('post')) {
            //Override displaying title based on blog option
            $show_title_area_meta = sweettooth_elated_get_meta_field_intersect('show_title_area_blog');
    ...
    add_filter('sweettooth_elated_filter_show_title_area', 'sweettooth_elated_blog_single_title');
}

if( !function_exists('sweettooth_elated_blog_single_title_params') ) {
    /**
     * Function that replaces title image with single post featured image on single post page
     *
     * @see sweettooth_elated_get_title() from title-functions.php file for list of available params
     *
     * @param array - array of default title values
     *
     * @return array of overridden values
     */
    function sweettooth_elated_blog_single_title_params($params) {
        ...
        $show_title_area_blog_single = sweettooth_elated_get_meta_field_intersect('show_title_area_blog', get_the_ID());
        ...
        if (!empty($background_image)) {
            ...
            $is_img_responsive = sweettooth_elated_get_meta_field_intersect('title_area_background_image_responsive', get_the_ID());
...
if(!function_exists('sweettooth_elated_post_has_thumbnail')) {
    /**
     * Function that adds class on title holder if full height title option is enabled for single post page
     *
     * @see sweettooth_elated_title_background_image_classes() in filter-functions.php
     *
     * @param $title_img string with title image url
     *
     * @return string with image
     */
    function sweettooth_elated_post_has_thumbnail($title_img) {
