<?php
namespace SweetToothElated\Modules\Header\Types;

use SweetToothElated\Modules\Header\Lib\HeaderType;
...
class HeaderCentered extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->logoAreaHeight     = sweettooth_elated_set_default_logo_height_for_header_types();
			$this->menuAreaHeight     = sweettooth_elated_set_default_menu_height_for_header_types();
			$this->mobileHeaderHeight = sweettooth_elated_set_default_mobile_menu_height_for_header_types();
			
			add_action( 'wp', array( $this, 'setHeaderHeightProps' ) );
			
			add_filter( 'sweettooth_elated_filter_js_global_variables', array( $this, 'getGlobalJSVariables' ) );
			add_filter( 'sweettooth_elated_filter_per_page_js_vars', array( $this, 'getPerPageJSVariables' ) );
		}
	}
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['logo_area_in_grid'] = sweettooth_elated_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' ? true : false;
		$parameters['menu_area_in_grid'] = sweettooth_elated_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' ? true : false;
		
		$parameters = apply_filters( 'sweettooth_elated_filter_header_centered_parameters', $parameters );
		
		sweettooth_elated_get_module_template_part( 'templates/' . $this->slug, $this->moduleName . '/types/' . $this->slug, '', $parameters );
	}
	...
	public function calculateHeightOfTransparency() {
		...
		$logo_background_color             = sweettooth_elated_options()->getOptionValue( 'logo_area_background_color' );
		$logo_background_transparency      = sweettooth_elated_options()->getOptionValue( 'logo_area_background_transparency' );
		$logo_grid_background_color        = sweettooth_elated_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = sweettooth_elated_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		$menu_background_color             = sweettooth_elated_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = sweettooth_elated_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = sweettooth_elated_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = sweettooth_elated_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( ( $sliderExists && sweettooth_elated_is_top_bar_enabled() )
				     || sweettooth_elated_is_top_bar_enabled() && sweettooth_elated_is_top_bar_transparent()
				) {
					$transparencyHeight += sweettooth_elated_get_top_bar_height();
				}
			}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( sweettooth_elated_is_top_bar_enabled() && sweettooth_elated_is_top_bar_completely_transparent() ) {
					$transparencyHeight += sweettooth_elated_get_top_bar_height();
				}
			}
	...
	public function calculateHeaderHeight() {
		...
		if ( sweettooth_elated_is_top_bar_enabled() ) {
			$headerHeight += sweettooth_elated_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = sweettooth_elated_get_meta_field_intersect( 'header_behaviour' );
		if ( ! in_array( $header_behavior, array( 'sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up' ) ) ) {
			$perPageVars['eltdfHeaderTransparencyHeight'] = $this->headerHeight - ( sweettooth_elated_get_top_bar_height() + $this->heightOfCompleteTransparency );
		} else {
