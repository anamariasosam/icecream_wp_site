<?php
class SweetToothElatedClassIconWidget extends SweetToothElatedClassWidget {
    /**
    ...
    public function __construct() {
        parent::__construct(
            'eltdf_icon_widget',
            esc_html__('Elated Icon Widget', 'sweettooth'),
            array( 'description' => esc_html__( 'Add icons to widget areas', 'sweettooth'))
        );
    ...
    protected function setParams() {
        $this->params = array_merge(
            sweettooth_elated_icon_collections()->getIconWidgetParamsArray(),
            array(
                array(
                    'type'  => 'textfield',
                    'name'  => 'link',
                ...
                array(
                    ...
                    'options' => array(
                        '_self'  => esc_html__('Same Window', 'sweettooth'),
                        '_blank' => esc_html__('New Window', 'sweettooth')
                    )
                ),
	            array(
		            'type'    => 'dropdown',
                ...
                array(
                    'type'  => 'textfield',
                    'name'  => 'icon_text',
                    'title' => esc_html__('Icon Text', 'sweettooth')
                ),
	            array(
		            'type'        => 'textfield',
		            'name'        => 'text_line_break',
		            'title'       => esc_html__('Break Text On Selected Lines', 'sweettooth'),
		            'description' => esc_html__( 'Enter the positions of the word after which you would like to create a line break. Separate the positions with commas (e.g. if you would like the line break after the 3rd word and 8th word, you would enter "3, 8")', 'sweettooth' ),
	            ),
	            array(
		            'type'  => 'textfield',
		            'name'  => 'text_color',
		            'title' => esc_html__('Text Color', 'sweettooth')
	            ),
	            array(
		            'type'    => 'dropdown',
		            'name'    => 'text_position',
		            'title'   => esc_html__( 'Icon Text Position', 'sweettooth' ),
		            'options'   => array(
			            'eltdf-text-left'  => esc_html__( 'Left From Icon', 'sweettooth' ),
			            'eltdf-text-right' => esc_html__( 'Right From Icon', 'sweettooth' ),
			            'eltdf-text-above' => esc_html__( 'Above Icon', 'sweettooth' ),
			            'eltdf-text-bellow' => esc_html__( 'Bellow Icon', 'sweettooth' )
		            )
	            ),
                array(
                    'type'  => 'textfield',
                    'name'  => 'icon_text_size',
                    'title' => esc_html__('Icon Text Size (px)', 'sweettooth')
                ),
                array(
                    'type'        => 'textfield',
                    'name'        => 'icon_text_margin',
                    'title'       => esc_html__('Icon Text Margin', 'sweettooth'),
                    'description' => esc_html__('Insert margin in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'sweettooth')
                )
            )
        );
    ...
    public function widget($args, $instance) {
        ...
        $icon_holder_html = '';
        $icon_class = array('eltdf-icon-widget');
	    ...
	    $text_styles = array();
	    if (!empty($instance['text_color']) && $instance['text_color'] !== '') {
		    $text_styles[] = 'color: '.$instance['text_color'];
	    }
	    if( ! empty($instance['icon_text_size']) ) {
		    $text_styles[] = 'font-size: '.sweettooth_elated_filter_px($instance['icon_text_size']).'px';
	    }
	    if( ! empty($instance['icon_text_margin']) ) {
		    $text_styles[] = 'margin: '.$instance['icon_text_margin'];
	    }

        $icon_text_html = '';
	    $widget_class = array();
        if(!empty($instance['icon_text'])) {
	        $text_break_words = !empty($instance['text_line_break']) ? $instance['text_line_break'] : '';
	        $split_text = explode( ' ', $instance['icon_text'] );
            ...
            $icon_text_html = '<span class="eltdf-icon-text-holder" '.sweettooth_elated_get_inline_style($text_styles).'><span class="eltdf-icon-text">'.$text.'</span></span>';
        }

	    $widget_class[] = !empty($instance['icon_hover_color']) || !empty($instance['shape_hover_color']) || !empty($instance['border_hover_color']) ? 'eltdf-icon-has-hover' : '';
	    ...
	    if( ! empty($instance['icon_margin']) ) {
		    $icon_styles[] = 'margin: '.$instance['icon_margin'];
	    }
        ...
        <a <?php sweettooth_elated_class_attribute($widget_class); ?> href="<?php echo esc_html($link); ?>" <?php echo wp_kses_post($data); ?> target="<?php echo esc_attr($target); ?>" <?php echo sweettooth_elated_get_inline_style($icon_styles); ?>>
			<?php
				if( $instance['text_position'] === 'eltdf-text-above' ) {
					echo wp_kses($icon_text_html, array('span' => array('class' => true, 'style' => true), 'br' => true)) . wp_kses($icon_holder_html, array('span' => array('class' => true, 'style' => true), 'i'  => array('class' => true)));
				} else {
					echo wp_kses($icon_holder_html, array('span' => array('class' => true, 'style' => true), 'i'  => array('class' => true))) . wp_kses($icon_text_html, array('span' => array('class' => true, 'style' => true), 'br' => true));
				}
			?>
        </a>
    <?php
