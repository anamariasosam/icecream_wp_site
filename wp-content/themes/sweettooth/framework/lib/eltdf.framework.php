...
/*
   Class: SweetToothElatedClassFramework
...
class SweetToothElatedClassFramework
{
    ...
    private function __construct() {
        $this->eltdOptions = SweetToothElatedClassOptions::get_instance();
        $this->eltdMetaBoxes = SweetToothElatedClassMetaBoxes::get_instance();
    }
    ...
    public function setSkin(SweetToothElatedClassSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class SweetToothElatedClassSkinManager
 *
...
/**
 * Class SweetToothElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class SweetToothElatedClassSkinAbstract
{
    /**
     * @var string
     */
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if ($template !== '') {
            include(sweettooth_elated_find_template_path('framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.eltdSCIcon = '<?php echo sweettooth_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
/*
   Class: SweetToothElatedClassOptions
...
class SweetToothElatedClassOptions
{
    ...
    public function getOptionValue($key) {
        ...
        if (array_key_exists($key, $sweettooth_php_global_options)) {
            return $sweettooth_php_global_options[$key];
...
/*
   Class: SweetToothElatedClassAdminPage
   A class that initializes Elated Admin Page
*/

class SweetToothElatedClassAdminPage implements iSweetToothElatedInterfaceLayoutNode
{
    ...
    function __construct($slug = "", $title = "", $icon = "") {
        $this->layout = array();
        $this->factory = new SweetToothElatedClassFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iSweetToothElatedInterfaceRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: SweetToothElatedClassMetaBoxes
...
/*
   Class: SweetToothElatedClassMetaBox
   A class that initializes Elated Meta Box
*/

class SweetToothElatedClassMetaBox implements iSweetToothElatedInterfaceLayoutNode
{
    ...
    public function setTitle($label) {
        ...
        return $sweettooth_php_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
    }
}

if (!function_exists('sweettooth_elated_init_framework_variable')) {
    function sweettooth_elated_init_framework_variable() {
        global $sweettooth_php_global_Framework;

        $sweettooth_php_global_Framework = SweetToothElatedClassFramework::get_instance();
        $eltdSkinManager = new SweetToothElatedClassSkinManager();
        $sweettooth_php_global_Framework->setSkin($eltdSkinManager->getSkin());
    }

    add_action('sweettooth_elated_action_before_options_map', 'sweettooth_elated_init_framework_variable');
}
?>