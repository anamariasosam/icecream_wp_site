...
class ElatedSkin extends SweetToothElatedClassSkinAbstract {
    /**
     * Skin constructor. Hooks to sweettooth_elated_admin_scripts_init and sweettooth_elated_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'elated';

        //hook to
        add_action('sweettooth_elated_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('sweettooth_elated_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('sweettooth_elated_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('sweettooth_elated_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('sweettooth_elated_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('sweettooth_elated_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $scriptPath) {
	        sweettooth_elated_register_skin_script($scriptHandle, $scriptPath);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        sweettooth_elated_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = sweettooth_elated_get_admin_tab();
        $active_page = $sweettooth_php_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
	 ...
	 *
	 * @param SweetToothElatedClassAdminPage $page current page to load
	 * @see ElatedSkinAbstract::loadTemplatePart()
	 */
