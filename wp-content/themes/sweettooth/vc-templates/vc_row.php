...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $simple_background_image =  $simple_background_image_1440 = $simple_background_image_1280 = $simple_background_image_1024 = $simple_background_image_768 = $simple_background_image_480 = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = '';
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		if ( ! empty( $simple_background_image_1440 ) ) {
			$hasStageImages = true;
			$image_src = wp_get_attachment_image_src($simple_background_image_1440, 'full');

			$wrapper_attributes[] = 'data-image-1440="'.$image_src[0].'"';
		}

		if ( ! empty( $simple_background_image_1280 ) ) {
			$hasStageImages = true;
			$image_src = wp_get_attachment_image_src($simple_background_image_1280, 'full');

			$wrapper_attributes[] = 'data-image-1280="'.$image_src[0].'"';
		}

		if ( ! empty( $simple_background_image_1024 ) ) {
			$hasStageImages = true;
			$image_src = wp_get_attachment_image_src($simple_background_image_1024, 'full');

			$wrapper_attributes[] = 'data-image-1024="'.$image_src[0].'"';
		}

		if ( ! empty( $simple_background_image_768 ) ) {
			$hasStageImages = true;
			$image_src = wp_get_attachment_image_src($simple_background_image_768, 'full');

			$wrapper_attributes[] = 'data-image-768="'.$image_src[0].'"';
		}

		if ( ! empty( $simple_background_image_480 ) ) {
			$hasStageImages = true;
			$image_src = wp_get_attachment_image_src($simple_background_image_480, 'full');

			$wrapper_attributes[] = 'data-image-480="'.$image_src[0].'"';
		}

		if( $hasStageImages ) {
			$rand = mt_rand(100000,1000000);
			$css_classes[] = 'eltdf-row-stage-images';
			$css_classes[] = 'eltdf-row-custom-'.$rand;

			$wrapper_attributes[] = 'data-row-custom-class="eltdf-row-custom-'.$rand.'"';
		}
	}
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		if ( ! empty( $simple_background_image_1440 ) ) {
			...
			$grid_row_data[] = 'data-image-1440='.$image_src[0];
		}
		...
		if ( ! empty( $simple_background_image_1280 ) ) {
			...
			$grid_row_data[] = 'data-image-1280='.$image_src[0];
		}
		...
		if ( ! empty( $simple_background_image_1024 ) ) {
			...
			$grid_row_data[] = 'data-image-1024='.$image_src[0];
		}
		...
		if ( ! empty( $simple_background_image_768 ) ) {
			...
			$grid_row_data[] = 'data-image-768='.$image_src[0].'';
		}
		...
		if ( ! empty( $simple_background_image_480 ) ) {
			...
			$grid_row_data[] = 'data-image-480='.$image_src[0].'';
		}

		if( $hasStageImages ) {
			$rand = mt_rand(100000,1000000);
			$grid_row_class[] = 'eltdf-row-stage-images';
			$grid_row_class[] = 'eltdf-row-custom-'.$rand;

			$grid_row_data[] = 'data-row-custom-class=eltdf-row-custom-'.$rand;
		}
	}
...
if($row_content_width === 'grid'){
	$eltdf_row_wrapper_start .= '<div class="eltdf-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . sweettooth_elated_get_inline_style( $eltdf_grid_row_styles ) . '><div class="eltdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . sweettooth_elated_get_inline_style( $eltdf_vc_row_styles ) . '>';
