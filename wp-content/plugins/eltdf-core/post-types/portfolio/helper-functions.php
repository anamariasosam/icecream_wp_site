...
if(!function_exists('eltdf_core_portfolio_meta_box_functions')) {
	...
	add_filter('sweettooth_elated_filter_meta_box_post_types_save', 'eltdf_core_portfolio_meta_box_functions');
	add_filter('sweettooth_elated_filter_meta_box_post_types_remove', 'eltdf_core_portfolio_meta_box_functions');
}
...
if(!function_exists('eltdf_core_portfolio_scope_meta_box_functions')) {
	...
	add_filter('sweettooth_elated_filter_set_scope_for_meta_boxes', 'eltdf_core_portfolio_scope_meta_box_functions');
}
...
if(!function_exists('eltdf_core_get_archive_portfolio_list')) {
	function eltdf_core_get_archive_portfolio_list($eltdf_taxonomy_slug = '', $eltdf_taxonomy_name = '') {
		...
		$number_of_items = 12;
		$number_of_items_option = sweettooth_elated_options()->getOptionValue('portfolio_archive_number_of_items');
		if(!empty($number_of_items_option)) {
			$number_of_items = $number_of_items_option;
		}
		
		$number_of_columns = 4;
		$number_of_columns_option = sweettooth_elated_options()->getOptionValue('portfolio_archive_number_of_columns');
		if(!empty($number_of_columns_option)) {
			$number_of_columns = $number_of_columns_option;
		}
		
		$space_between_items = 'normal';
		$space_between_items_option = sweettooth_elated_options()->getOptionValue('portfolio_archive_space_between_items');
		if(!empty($space_between_items_option)) {
			$space_between_items = $space_between_items_option;
		}
		
		$image_size = 'landscape';
		$image_size_option = sweettooth_elated_options()->getOptionValue('portfolio_archive_image_size');
		if(!empty($image_size_option)) {
			$image_size = $image_size_option;
		}
		
		$item_layout = 'standard-shader';
		$item_layout_option = sweettooth_elated_options()->getOptionValue('portfolio_archive_item_layout');
		...
		$html = sweettooth_elated_execute_shortcode('eltdf_portfolio_list', $params);
...
if(!function_exists('eltdf_core_set_portfolio_single_info_follow_body_class')) {
	...
	function eltdf_core_set_portfolio_single_info_follow_body_class($classes) {
		if(is_singular('portfolio-item') && sweettooth_elated_options()->getOptionValue('portfolio_single_sticky_sidebar') == 'yes'){
...
if(!function_exists('eltdf_core_single_portfolio_title_display')) {
    ...
    function eltdf_core_single_portfolio_title_display($show_title_area) {
        if(is_singular('portfolio-item')) {
            ...
            $show_title_area_meta = sweettooth_elated_get_meta_field_intersect('show_title_area_portfolio_single');
    ...
    add_filter('sweettooth_elated_filter_show_title_area', 'eltdf_core_single_portfolio_title_display');
}

if(!function_exists('eltdf_core_get_single_portfolio')) {
	function eltdf_core_get_single_portfolio() {
		$portfolio_template = sweettooth_elated_get_meta_field_intersect('portfolio_single_template');
...
if( !function_exists('eltdf_core_set_single_portfolio_style') ) {
	...
	function eltdf_core_set_single_portfolio_style($style) {
		...
		$class_prefix = sweettooth_elated_get_unique_page_class($id);
		...
		if(!empty($info_padding_top)) {
			$current_style['padding-top'] = sweettooth_elated_filter_px($info_padding_top).'px';
			
			$current_styles .= sweettooth_elated_dynamic_css($current_selector, $current_style);
		}
	...
	add_filter('sweettooth_elated_filter_add_page_custom_style', 'eltdf_core_set_single_portfolio_style');
}
...
if(!function_exists('eltdf_core_get_portfolio_single_media_html')) {
	function eltdf_core_get_portfolio_single_media_html($media) {
		...
		if($media['type'] == 'image') {
			$params['lightbox'] = sweettooth_elated_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
		...
		if(in_array($media['type'], array('youtube', 'vimeo'))) {
			$params['lightbox'] = sweettooth_elated_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
