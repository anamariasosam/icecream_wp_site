...
if(!function_exists('sweettooth_elated_get_social_share_html')) {
	/**
	 ...
	 * @return mixed|string
	 */
	function sweettooth_elated_get_social_share_html($params = array()) {
        if(sweettooth_elated_core_plugin_installed()) {
	        return sweettooth_elated_execute_shortcode( 'eltdf_social_share', $params );
        }
	}
}

if (!function_exists('sweettooth_elated_the_excerpt_max_charlength')) {
	/**
	 * Function that sets character length for social share shortcode
	 * @param $charlength string original text
	 * @return string shortened text
	 */
	function sweettooth_elated_the_excerpt_max_charlength($charlength) {

		if (sweettooth_elated_options()->getOptionValue('twitter_via')) {
			$via = ' via ' . esc_attr(sweettooth_elated_options()->getOptionValue('twitter_via'));
		} else {
			$via = '';
		}
