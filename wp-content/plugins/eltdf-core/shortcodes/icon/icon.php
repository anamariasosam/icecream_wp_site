...
class Icon implements Lib\ShortcodeInterface {
	    ...
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    ...
				    'params'                    => array_merge(
					    \SweetToothElatedClassIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
    ...
    public function render($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, sweettooth_elated_icon_collections()->getShortcodeParams());
        $params       = shortcode_atts($default_atts, $atts);

        $iconPackName = sweettooth_elated_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
	    ...
	    if(!empty($params['hover_icon_color']) || !empty($params['hover_border_color']) || !empty($params['hover_background_color'])) {
		    $iconHolderClasses[] = 'eltdf-icon-has-hover';
	    }
    ...
    private function generateIconStyles($params) {
        ...
        if(($params['type'] !== 'eltdf-normal' && !empty($params['shape_size'])) || ($params['type'] == 'eltdf-normal')) {
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.sweettooth_elated_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'eltdf-normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.sweettooth_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.sweettooth_elated_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.sweettooth_elated_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.sweettooth_elated_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'eltdf-square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.sweettooth_elated_filter_px($params['border_radius']).'px';
                }
            }
        }
