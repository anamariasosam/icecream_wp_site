...
class Counter implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Digit Side Padding (px or %)', 'eltdf-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
	...
	private function getCounterStyles($params) {
		...
		if (!empty($params['digit_font_size'])) {
			$styles[] = 'font-size: '.sweettooth_elated_filter_px($params['digit_font_size']).'px';
		}
        ...
        if(!empty($params['digit_padding'])) {
	        $styles[] = 'padding: 0 '.$params['digit_padding'];
        }
