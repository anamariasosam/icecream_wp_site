...
class ElatedfCoreImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        $result = update_option( 'eltdf_options_sweettooth', $options);
    ...
    function eltdf_admin_import() {
        if (eltdf_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($sweettooth_php_global_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'sweettooth_elated_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'sweettooth_elated_enqueue_admin_styles');
        }
    }
}