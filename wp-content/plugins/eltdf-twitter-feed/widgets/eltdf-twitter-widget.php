...
class ElatedfTwitterWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			esc_html__( 'Elated Twitter Widget', 'eltdf-twitter-feed' ),
			array(
				'description' => esc_html__( 'Display your Twitter feed', 'eltdf-twitter-feed' )
			)
		);
	...
	private function setParams() {
		$this->params = array(
			array(
				'name'  => 'title',
				'type'  => 'textfield',
				'title' => esc_html__( 'Title', 'eltdf-twitter-feed' )
			),
			array(
				'name'    => 'type_of_widget',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'eltdf-twitter-feed' ),
				'options' => array(
					'standard' => esc_html__( 'Standard', 'eltdf-twitter-feed' ),
					'slider'   => esc_html__( 'Slider', 'eltdf-twitter-feed' )
				)
			),
			array(
				'name'  => 'user_id',
				'type'  => 'textfield',
				'title' => esc_html__( 'User ID', 'eltdf-twitter-feed' )
			),
			array(
				'name'  => 'count',
				'type'  => 'textfield',
				'title' => esc_html__( 'Number of Tweets', 'eltdf-twitter-feed' )
			),
			array(
				'name'    => 'show_tweet_icon',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Icon', 'eltdf-twitter-feed' ),
				'options' => array(
					'yes' => esc_html__( 'Yes', 'eltdf-twitter-feed' ),
					'no'  => esc_html__( 'No', 'eltdf-twitter-feed' )
				)
			),
			array(
				'name'    => 'show_tweet_time',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Time', 'eltdf-twitter-feed' ),
				'options' => array(
					'no'  => esc_html__( 'No', 'eltdf-twitter-feed' ),
					'yes' => esc_html__( 'Yes', 'eltdf-twitter-feed' )
				)
			...
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__( 'Tweets Cache Time', 'eltdf-twitter-feed' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$holder_classes = 'eltdf-twitter-' . esc_attr( $type_of_widget );
		$slider_data    = array();
		
		if ( $type_of_widget === 'slider' ) {
			$holder_classes .= ' eltdf-owl-slider';
		...
		if ( $twitter_api->hasUserConnected() ) {
			...
			if ( $response->status ) {
				if ( is_array( $response->data ) && count( $response->data ) ) { ?>
					<ul class="eltdf-twitter-widget <?php echo esc_attr( $holder_classes ); ?>" <?php echo sweettooth_elated_get_inline_attrs( $slider_data ); ?>>
						<?php foreach ( $response->data as $tweet ) { ?>
							
							<?php if ( $type_of_widget == 'slider' ) { ?>
								
								<li class="eltdf-tweet-holder">
									<div class="eltdf-tweet-text" <?php sweettooth_elated_inline_style( $twitter_style ); ?>>
										...
										<?php if ( $show_tweet_time == 'yes' ) { ?>
											<a class="eltdf-tweet-time" target="_blank" href="<?php echo esc_url( $twitter_api->getHelper()->getTweetURL( $tweet ) ); ?>">
												<?php if ( $show_tweet_icon == 'yes' ) { ?>
													<span class="eltdf-twitter-icon"><i class="social_twitter"></i></span>
												<?php } ?>
							...
							<?php } else { ?>
								...
								<li class="eltdf-tweet-holder">
									<?php if ( $show_tweet_icon == 'yes' ) { ?>
										<div class="eltdf-twitter-icon"><i class="social_twitter"></i></div>
									<?php } ?>
									<div class="eltdf-tweet-text" <?php sweettooth_elated_inline_style( $twitter_style ); ?>>
										<?php echo wp_kses_post( $twitter_api->getHelper()->getTweetText( $tweet ) ); ?>
										<?php if ( $show_tweet_time == 'yes' ) { ?>
